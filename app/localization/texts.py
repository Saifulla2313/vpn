from typing import Dict, Any


class TextsWrapper:
    """Wrapper for texts dictionary with attribute access and t() method."""
    
    def __init__(self, texts_dict: Dict[str, Any]):
        self._texts = texts_dict
    
    def __getattr__(self, name: str) -> str:
        if name.startswith('_'):
            raise AttributeError(name)
        return self._texts.get(name.lower(), name)
    
    def __getitem__(self, key: str) -> str:
        return self._texts.get(key, key)
    
    def get(self, key: str, default: str = "") -> str:
        return self._texts.get(key, default)
    
    def t(self, key: str, default: str = "") -> str:
        """Get text by key with a default fallback."""
        return self._texts.get(key.lower(), default)
    
    # Admin panel text
    ADMIN_PANEL = """
🔧 <b>Панель администратора</b>

Выберите раздел для управления:
"""


TEXTS = {
    "ru": {
        "welcome": """
👋 <b>Добро пожаловать в VPN сервис!</b>

🔐 Безопасный и быстрый VPN для защиты вашего интернет-соединения.

💰 <b>Стоимость:</b> 6₽/день
   (пополняете баланс - подписка продлевается автоматически)

🎁 <b>Пробный период:</b> 1 день бесплатно!

Нажмите кнопку ниже, чтобы начать:
""",
        "profile": """
👤 <b>Ваш профиль</b>

🆔 ID: <code>{telegram_id}</code>
💰 Баланс: <b>{balance:.2f}₽</b>
📅 Подписка до: <b>{expires}</b>
📊 Статус: <b>{status}</b>

💡 Стоимость: 6₽/день
📈 Оплачено дней: {days_paid}
""",
        "subscription_active": """
✅ <b>Ваша VPN подписка активна!</b>

📅 Действует до: <b>{expires}</b>
💰 Ваш баланс: <b>{balance:.2f}₽</b>

При достаточном балансе подписка продлевается автоматически.
""",
        "subscription_inactive": """
❌ <b>VPN подписка неактивна</b>

💰 Ваш баланс: <b>{balance:.2f}₽</b>

Пополните баланс минимум на 6₽ для активации подписки.
""",
        "deposit_menu": """
💰 <b>Пополнение баланса</b>

Текущий баланс: <b>{balance:.2f}₽</b>
Стоимость подписки: <b>6₽/день</b>

Выберите сумму пополнения:
""",
        "payment_created": """
💳 <b>Счет на оплату создан</b>

Сумма: <b>{amount}₽</b>

Нажмите кнопку "Оплатить" для перехода к оплате.
После оплаты нажмите "Я оплатил" для проверки.
""",
        "payment_success": """
✅ <b>Оплата успешна!</b>

Сумма: <b>{amount:.2f}₽</b>
Новый баланс: <b>{balance:.2f}₽</b>

{subscription_info}
""",
        "payment_pending": """
⏳ Платеж еще обрабатывается. Подождите немного и попробуйте снова.
""",
        "payment_failed": """
❌ Платеж не найден или не был оплачен. Попробуйте еще раз.
""",
        "trial_activated": """
🎁 <b>Пробный период активирован!</b>

Ваша VPN подписка активна на 1 день.
Не забудьте пополнить баланс для продолжения!
""",
        "trial_already_used": """
❌ Вы уже использовали пробный период.

Пополните баланс для активации подписки.
""",
        "vpn_key": """
🔐 <b>Ваш VPN ключ:</b>

<code>{subscription_url}</code>

📱 <b>Инструкции по подключению:</b>

1. Скачайте приложение V2RayNG (Android) или Streisand (iOS)
2. Скопируйте ключ выше
3. В приложении нажмите "+" и выберите "Импорт из буфера"
4. Подключитесь!

🔗 Или перейдите по ссылке: {subscription_url}
""",
        "help": """
❓ <b>Справка</b>

<b>Как это работает:</b>
• Пополните баланс любой суммой
• Подписка стоит 6₽/день
• Каждый день списывается 6₽ с баланса
• Если баланс &lt; 6₽, подписка приостанавливается
• При пополнении баланса подписка возобновляется

<b>Поддержка:</b>
Если у вас возникли проблемы, напишите администратору.
""",
        "admin_panel": """
🔧 <b>Панель администратора</b>

👥 Пользователей: {users_count}
✅ Активных подписок: {active_count}
💰 Общий баланс: {total_balance:.2f}₽
""",
        "error": "❌ Произошла ошибка. Попробуйте позже.",
        "maintenance": "🔧 Сервис временно недоступен. Попробуйте позже."
    }
}


def get_texts(language: str = "ru") -> TextsWrapper:
    texts_dict = TEXTS.get(language, TEXTS["ru"])
    return TextsWrapper(texts_dict)


def get_text(key: str, language: str = "ru", **kwargs) -> str:
    texts = get_texts(language)
    text = texts.get(key, key)
    if kwargs:
        try:
            text = text.format(**kwargs)
        except KeyError:
            pass
    return text


_rules_cache: Dict[str, Any] = {}


def clear_rules_cache() -> None:
    """Clear the rules cache."""
    global _rules_cache
    _rules_cache = {}
