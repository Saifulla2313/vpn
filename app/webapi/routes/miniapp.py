...
from ..schemas.miniapp import (
    ...
    MiniAppSubscriptionPurchasePaymentRequest,
    MiniAppSubscriptionPurchasePaymentResponse,
...
@router.post(
    ...
    response_model=MiniAppSubscriptionPurchasePaymentResponse,
)
async def subscription_purchase_payment_endpoint(
    payload: MiniAppSubscriptionPurchasePaymentRequest,
    db: AsyncSession = Depends(get_db_session),
) -> MiniAppSubscriptionPurchasePaymentResponse:
    """
    Create a direct payment for subscription purchase (bypassing balance).
    This allows users to pay for subscription directly via YooKassa or other payment methods.
    """
    user = await _authorize_miniapp_user(payload.init_data, db)
    context = await purchase_service.build_options(db, user)
    ...
    # Get the total price from pricing
    total_kopeks = getattr(pricing, "final_total", None) or getattr(pricing, "total", 0)
    if not total_kopeks or total_kopeks <= 0:
        raise HTTPException(
            status.HTTP_400_BAD_REQUEST,
            detail={"code": "invalid_price", "message": "Could not calculate subscription price"},
        )
    ...
    if not payment_method:
        raise HTTPException(
            status.HTTP_400_BAD_REQUEST,
            detail={"code": "missing_payment_method", "message": "Payment method is required"},
        )

    # Get period days for description
    period_days = getattr(getattr(selection, "period", None), "days", None) or 30
    
    # Create subscription order
    from app.services.subscription_purchase_service import SubscriptionPurchaseService
    sub_purchase_service = SubscriptionPurchaseService()
    
    # Get selection details
    device_limit = getattr(selection, "devices", 1)
    traffic_limit_gb = getattr(getattr(selection, "traffic", None), "value", 0) or 0
    server_uuids = list(getattr(selection, "servers", set()) or set())
    squad_uuid = server_uuids[0] if server_uuids else None
    
    order = await sub_purchase_service.create_subscription_order(
        db=db,
        user_id=user.id,
        ...
        total_price_kopeks=total_kopeks,
    )
    
    if not order:
        raise HTTPException(
            status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail={"code": "order_creation_failed", "message": "Failed to create subscription order"},
        )
    ...
    if payment_method == "yookassa_sbp":
        if not settings.is_yookassa_enabled() or not getattr(settings, "YOOKASSA_SBP_ENABLED", False):
            raise HTTPException(
                status.HTTP_400_BAD_REQUEST,
                detail={"code": "method_unavailable", "message": "YooKassa SBP is not available"},
            )
        ...
        payment_result = await payment_service.create_yookassa_sbp_payment(
            ...
            description=f"Оплата подписки на {period_days} дней",
            metadata={
                "user_telegram_id": str(user.telegram_id),
                "user_username": user.username or "",
                "order_id": str(order.id),
                "subscription_period": str(period_days),
        ...
        if not settings.is_yookassa_enabled():
            raise HTTPException(
                status.HTTP_400_BAD_REQUEST,
                detail={"code": "method_unavailable", "message": "YooKassa is not available"},
            )
        ...
        raise HTTPException(
            status.HTTP_400_BAD_REQUEST,
            detail={"code": "unsupported_method", "message": f"Payment method '{payment_method}' is not supported for direct subscription purchase"},
        )
    ...
    if not confirmation_url:
        raise HTTPException(
            status.HTTP_502_BAD_GATEWAY,
            detail={"code": "no_payment_url", "message": "Payment URL not received"},
        )

    return MiniAppSubscriptionPurchasePaymentResponse(
        payment_url=confirmation_url,
        order_id=order.id,
        payment_id=payment_result.get("yookassa_payment_id"),
        local_payment_id=payment_result.get("local_payment_id"),
        amount_kopeks=total_kopeks,
        amount_label=settings.format_price(total_kopeks),
        extra={
            "period_days": period_days,
            "device_limit": device_limit,
            "traffic_limit_gb": traffic_limit_gb,
