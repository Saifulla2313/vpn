...
async def show_welcome_text_panel(
    ...
    await callback.message.edit_text(
        ...
        reply_markup=get_welcome_text_keyboard(db_user.language_code, welcome_settings['is_enabled']),
        parse_mode="HTML"
    )
...
async def toggle_welcome_text(
    ...
    await callback.message.edit_text(
        ...
        f"Здесь вы можете управлять текстом, который показывается новым пользователям после регистрации.\n\n"
        f"💡 Доступные плейсхолдеры для автозамены:",
        reply_markup=get_welcome_text_keyboard(db_user.language_code, new_status),
        parse_mode="HTML"
    )
    await callback.answer()
...
async def show_current_welcome_text(
    ...
    await callback.message.edit_text(
        ...
        reply_markup=get_welcome_text_keyboard(db_user.language_code, is_enabled),
        parse_mode="HTML"
    )
    await callback.answer()
